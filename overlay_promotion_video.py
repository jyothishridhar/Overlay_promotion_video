...
def generate_overlay_excel_report(reference_overlay_frames, testing_overlay_frames, report_path):
    ...
    for row_num in range(2, max_length + 2):
        if row_num <= len(reference_overlay_frames):
            ref_timestamp, ref_frame_num = reference_overlay_frames[row_num - 2]
        ...
        if row_num <= len(testing_overlay_frames):
            test_timestamp, test_frame_num = testing_overlay_frames[row_num - 2]
...
if st.button("Run Overlay Detection"):
    if reference_video_path is not None and testing_video_path is not None and report_path:
        ...
        generate_overlay_excel_report(reference_overlay_frames, testing_overlay_frames, report_path)
